...
class Pass(models.Model):
	...
	def get_teachers_old_passes(user):
		...
		if profile.is_teacher:
			...
			# teacher_pass = TeacherPass.objects.filter(approved=True, destinationTeacher=teacher).exclude(timeArrivedDestination=None)
			return Pass.objects.filter(approved=True, originTeacher=teacher).exclude(timeArrivedDestination=None) | Pass.objects.filter(approved=True, destinationTeacher=teacher)
			# return teacher_pass | origin_pass
		else:
			return None
